...
-- заполнил фейкером таблицу posts, поля user_id и community_id заполнены везде, но пост может принадлежать либо user либо community    
DESC posts;
...
UPDATE posts SET community_id = (SELECT id FROM communities_temp ORDER BY RAND() LIMIT 1); -- заполнил community_id рандомно со значениями NULL

UPDATE posts SET user_id = NULL WHERE community_id > 0; -- в зависимости от значения community_id проставил NULL в user_id
...
SELECT target_id FROM likes 
  WHERE target_type_id = 
   (SELECT id FROM target_types WHERE name = 'users'); -- выборка лайков пользователям
